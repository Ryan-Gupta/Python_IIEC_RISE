...
run_list = [' run ',' open ',' execute ',' show ']
neg_list = [' dont ',' dont ','do not ',' not ']
exitc = [' exit ',' quit ',' stop ',' leave ']
...
#check if input contains any word from the file list, run that program & set file variable
	...
	if not file:
		...
		print('Press a to add the program to our list or enter to ignore & continue.')
		to_add = input()
		if (to_add is 'a'):
